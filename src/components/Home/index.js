...
export default function Home() {
            ...
            {tshirtList.length > 0 ? (
                tshirtList.map(eachItem => (
                    <li className="list-item" key={eachItem.id}>
                        <img className="image" src={eachItem.imageUrl} alt={eachItem.name}/>
                        <h5 className="name">{eachItem.name}</h5>
                        <div className="bottom-container">
                        <p className="price"><FaRupeeSign /> {eachItem.price}</p>
  ...
  return (
    <div>
      ...
      <button className="search-button-container"><IoIosSearch className="search-icon"/></button>
      <button className="filter-button"><CiFilter /></button>
      </div>
      </div>
